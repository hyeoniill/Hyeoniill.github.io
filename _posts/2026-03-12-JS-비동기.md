...
자바스크립트는 **싱글스레드** 언어이다. 따라서 자바스크립트 엔진 내부에서는 한 번에 하나의 작업만 실행할 수 있다.

자바와 파이썬 같은 다른 백엔드 언어는 멀티스레드를 지원한다. 

예를 들어 서버 환경에서는 오래 걸리는 I/O 작업이 실행되는 동안 스레드가 블로킹되면 다른 클라이언트의 요청을 처리하지 못하는 문제가 발생한다.
...
### 자바스크립트는 왜 싱글 스레드로 사용하는가?



작업의 성능은 `작업의 성격`에 따라서 싱글스레드의 성능이 멀티스레드 보다 높을 수 있다.

우선 서버를 생각해보자. 서버는 I/O작업이 대부분이다. 

I/O작업은 CPU가 하지 않는다. 너무 느린 작업이기 때문에 자원 낭비이다. 그래서 I/O작업은 컨트롤러에서 하게 된다.

CPU는 I/O작업이 들어오면 컨트롤러에 작업을 지시하고 다른 작업을 수행한다. 컨트롤러는 작업을 마무리하면 인터럽트를 발생시켜서 CPU에게 알린다. 그럼 CPU는 해당 작업을 다시 실행한다. 이는 OS 레벨에서 이루어지는 비동기 처리 방식이다.

그러나 소프트웨어 입장에서 보면 I/O가 끝날 때까지 작업이 멈춰져있기 때문에 동기화된 작업이라고 생각할 수 있다.
...
그래서 실제로는 data를 입력 받도록 실행시킨 후, `console.log(data);`는 콜백으로 넘긴다.
...
Promise는 비동기 작업의 상태(대기 → 성공/실패)를 표현하는 객체이다.

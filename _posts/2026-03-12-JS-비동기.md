...
---
자바스크립트는 **싱글스레드** 언어이다.  
따라서 자바스크립트 엔진 내부에서는 한 번에 하나의 작업만 실행된다.

반면 `자바`와 `파이썬` 같은 다른 백엔드 언어는 **멀티스레드**를 지원한다. 
...
I/O작업은 CPU의 속도에 비해서 너무 느린 작업이기 때문에 CPU가 직접 수행하지 않는다. 대신 디스크 컨트롤러 같은 하드웨어 컨트롤러에서 하게 된다.
